...
function displayCurationForm(draftId, draftData) {
    ...
    <div class="admin-curation-modal-content">
      ...
      <form data-draft-id="${draftId}" onsubmit="saveCurationDraft('${draftId}'); return false;">
        ...
        <fieldset class="admin-curation-fieldset">
          ...
          <div class="admin-curation-form-group">
            <label for="curation-safety-${draftId}">Legacy Safety Rating (v1)</label>
            <p style="font-size: 0.85rem; color: var(--text-gray); margin-bottom: 0.5rem;">Existing rating system. Keep for v1 apps during migration.</p>
        ...
        <fieldset class="admin-curation-fieldset">
          ...
          <details class="admin-exposure-details">
            <summary style="cursor: pointer; font-weight: 600; padding: 0.75rem; background: var(--bg-light); border-radius: 6px; margin-bottom: 1rem; user-select: none;">▶ Base Digital Exposure Level</summary>
            <div style="padding: 1rem; background: var(--bg-light); border-radius: 6px; margin-bottom: 1rem;">
              <div class="admin-curation-form-group">
                <label for="curation-exposure-level-${draftId}">Digital Exposure Level</label>
                <select id="curation-exposure-level-${draftId}" style="width: 100%; padding: 0.5rem;">
                  <option value="">-- Not Set --</option>
                  <option value="lower" ${draftData.exposureLevel === 'lower' ? 'selected' : ''}>🟢 Lower Exposure</option>
                  <option value="moderate" ${draftData.exposureLevel === 'moderate' ? 'selected' : ''}>🟡 Moderate Exposure</option>
                  <option value="higher" ${draftData.exposureLevel === 'higher' ? 'selected' : ''}>🟠 Higher Exposure</option>
                  <option value="very_high" ${draftData.exposureLevel === 'very_high' ? 'selected' : ''}>🔴 Very High Exposure</option>
                </select>
                <p style="font-size: 0.85rem; color: var(--text-gray); margin-top: 0.5rem;">Rate in normal/default configuration, not worst case.</p>
              </div>
              <div class="admin-curation-form-group">
                <label for="curation-exposure-explain-${draftId}">Exposure Explanation</label>
                <textarea id="curation-exposure-explain-${draftId}" maxlength="300" placeholder="Why does this level fit the app?" style="width: 100%; min-height: 80px; padding: 0.5rem;">${escapeHtml(draftData.exposureExplanation || '')}</textarea>
              </div>
            </div>
          </details>

          <!-- Exposure Factors -->
          <details class="admin-exposure-details" open>
            <summary style="cursor: pointer; font-weight: 600; padding: 0.75rem; background: var(--bg-light); border-radius: 6px; margin-bottom: 1rem; user-select: none;">▶ Exposure Factors</summary>
            <div style="padding: 1rem; background: var(--bg-light); border-radius: 6px; margin-bottom: 1rem;">
              <p style="font-size: 0.85rem; color: var(--text-gray); margin-bottom: 1rem;">Select factors present. Mark severity for each. Leave unchecked if not present.</p>
              <div id="curation-factors-container-${draftId}" style="display: grid; gap: 0.75rem;">
          ...
          <!-- Exposure Floor -->
          <details class="admin-exposure-details">
            <summary style="cursor: pointer; font-weight: 600; padding: 0.75rem; background: var(--bg-light); border-radius: 6px; margin-bottom: 1rem; user-select: none;">▶ Exposure Floor</summary>
            <div style="padding: 1rem; background: var(--bg-light); border-radius: 6px; margin-bottom: 1rem;">
              <div class="admin-curation-form-group">
                <label for="curation-exposure-floor-${draftId}">Exposure Floor</label>
                <select id="curation-exposure-floor-${draftId}" style="width: 100%; padding: 0.5rem;">
                  <option value="">-- Not Set --</option>
                  <option value="lower" ${draftData.exposureFloor === 'lower' ? 'selected' : ''}>🟢 Lower Exposure</option>
                  <option value="moderate" ${draftData.exposureFloor === 'moderate' ? 'selected' : ''}>🟡 Moderate Exposure</option>
                  <option value="higher" ${draftData.exposureFloor === 'higher' ? 'selected' : ''}>🟠 Higher Exposure</option>
                  <option value="very_high" ${draftData.exposureFloor === 'very_high' ? 'selected' : ''}>🔴 Very High Exposure</option>
                </select>
                <p style="font-size: 0.85rem; color: var(--text-gray); margin-top: 0.5rem;">Lowest level possible while core functionality remains.</p>
              </div>
              <div class="admin-curation-form-group">
                <label for="curation-floor-rationale-${draftId}">Floor Rationale</label>
                <textarea id="curation-floor-rationale-${draftId}" maxlength="300" placeholder="Why can't exposure go lower?" style="width: 100%; min-height: 80px; padding: 0.5rem;">${escapeHtml(draftData.floorRationale || '')}</textarea>
              </div>
            </div>
          </details>
          ...
          <details class="admin-exposure-details">
            <summary style="cursor: pointer; font-weight: 600; padding: 0.75rem; background: var(--bg-light); border-radius: 6px; margin-bottom: 1rem; user-select: none;">▶ Recommended Safeguards</summary>
            <div style="padding: 1rem; background: var(--bg-light); border-radius: 6px; margin-bottom: 1rem;">
              <div id="curation-safeguards-container-${draftId}" style="display: grid; gap: 1rem; margin-bottom: 1rem;">
                <!-- Safeguards will be populated by JavaScript -->
              </div>
              <button type="button" class="admin-curation-btn" onclick="addSafeguard('${draftId}')" style="width: 100%; background: var(--secondary); color: white;">+ Add Safeguard</button>
            </div>
          </details>

          <!-- Protected Exposure -->
          <details class="admin-exposure-details">
            <summary style="cursor: pointer; font-weight: 600; padding: 0.75rem; background: var(--bg-light); border-radius: 6px; margin-bottom: 1rem; user-select: none;">▶ Protected Exposure Level</summary>
            <div style="padding: 1rem; background: var(--bg-light); border-radius: 6px; margin-bottom: 1rem;">
              <div class="admin-curation-form-group">
                <label for="curation-protected-level-${draftId}">Protected Level (with recommended safeguards)</label>
                <select id="curation-protected-level-${draftId}" style="width: 100%; padding: 0.5rem;">
                  <option value="">-- Not Set --</option>
                  <option value="lower" ${draftData.protectedExposureLevel === 'lower' ? 'selected' : ''}>🟢 Lower Exposure</option>
                  <option value="moderate" ${draftData.protectedExposureLevel === 'moderate' ? 'selected' : ''}>🟡 Moderate Exposure</option>
                  <option value="higher" ${draftData.protectedExposureLevel === 'higher' ? 'selected' : ''}>🟠 Higher Exposure</option>
                  <option value="very_high" ${draftData.protectedExposureLevel === 'very_high' ? 'selected' : ''}>🔴 Very High Exposure</option>
                </select>
                <p style="font-size: 0.85rem; color: var(--text-gray); margin-top: 0.5rem;">Must eventually be between Floor and Base Exposure Level.</p>
              </div>
            </div>
          </details>
          ...
          <!-- What Remains -->
          <details class="admin-exposure-details">
            <summary style="cursor: pointer; font-weight: 600; padding: 0.75rem; background: var(--bg-light); border-radius: 6px; margin-bottom: 1rem; user-select: none;">▶ What Remains (Residual Exposure)</summary>
            <div style="padding: 1rem; background: var(--bg-light); border-radius: 6px; margin-bottom: 1rem;">
              <div id="curation-residual-container-${draftId}" style="display: grid; gap: 1rem; margin-bottom: 1rem;">
                <!-- Residual exposures will be populated by JavaScript -->
              </div>
              <button type="button" class="admin-curation-btn" onclick="addResidualExposure('${draftId}')" style="width: 100%; background: var(--secondary); color: white;">+ Add Remaining Exposure</button>
            </div>
          </details>
        </fieldset>
  ...
  // Initialize Digital Exposure UI
  initializeExposureFactors(draftId, draftData);
  initializeSafeguards(draftId, draftData);
  initializeResidualExposure(draftId, draftData);
}

/**
 * Initialize exposure factors UI
 */
function initializeExposureFactors(draftId, draftData) {
  if (!window.DigitalCapExposure) {
    console.warn('DigitalCapExposure not available');
    return;
  }

  const container = document.getElementById(`curation-factors-container-${draftId}`);
  if (!container) return;

  const config = window.DigitalCapExposure;
  const selectedFactors = draftData.exposureFactors || [];
  const selectedMap = {};
  selectedFactors.forEach(f => {
    selectedMap[f.factorKey] = f;
  });

  let html = '';
  config.factorKeys.forEach(factorKey => {
    const factor = config.factors[factorKey];
    const isSelected = !!selectedMap[factorKey];
    const selectedData = selectedMap[factorKey];
    const severity = selectedData ? selectedData.severity : '';
    const evidence = selectedData ? (selectedData.evidence || '') : '';

    html += `
      <div class="admin-exposure-factor" style="border: 1px solid var(--border-color); border-radius: 6px; padding: 0.75rem;">
        <label style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer; font-weight: 500;">
          <input type="checkbox" class="admin-factor-checkbox" data-factor-key="${factorKey}" data-draft-id="${draftId}" ${isSelected ? 'checked' : ''} onchange="toggleFactor(this)">
          ${factor.label}
        </label>
        <div class="admin-factor-details" style="display: ${isSelected ? 'block' : 'none'}; margin-top: 0.75rem; padding: 0.75rem; background: var(--bg-white); border-radius: 4px;">
          <div style="margin-bottom: 0.75rem;">
            <label style="display: block; font-size: 0.9rem; margin-bottom: 0.25rem;">Severity</label>
            <select class="admin-factor-severity" data-draft-id="${draftId}" data-factor-key="${factorKey}" style="width: 100%; padding: 0.5rem;">
          ...
          <div>
            <label style="display: block; font-size: 0.9rem; margin-bottom: 0.25rem;">Evidence / Notes</label>
            <textarea class="admin-factor-evidence" data-draft-id="${draftId}" data-factor-key="${factorKey}" maxlength="300" placeholder="What evidence supports this factor?" style="width: 100%; min-height: 60px; padding: 0.5rem; font-size: 0.85rem;">${escapeHtml(evidence)}</textarea>
          </div>
        </div>
      </div>
    `;
  ...
  // Special guidance for location_personal_data
  html += `
    <div style="background: rgba(255,183,3,0.1); border: 1px solid rgba(255,183,3,0.3); border-radius: 6px; padding: 0.75rem; margin-top: 1rem; font-size: 0.85rem;">
      <strong>Location & Personal Information Note:</strong><br>
      Consider: (A) What information does the service collect? (B) Can other users see live/recent location or identifying info? (C) Can profile fields be visible publicly or socially? Capture these distinctions in Evidence.
    </div>
  `;

  // Special guidance for content_persistence
  html += `
    <div style="background: rgba(255,140,66,0.1); border: 1px solid rgba(255,140,66,0.3); border-radius: 6px; padding: 0.75rem; margin-top: 0.75rem; font-size: 0.85rem;">
      <strong>Content Persistence Note:</strong><br>
      Measure user control after sharing: deletion, copying, screenshots, redistribution, disappearance expectations, content beyond user control. Not about "persistent=bad" or "disappearing=good."
    </div>
  `;
...
/**
 * Toggle exposure factor selection
 */
window.toggleFactor = function(checkbox) {
  const detailsDiv = checkbox.closest('.admin-exposure-factor').querySelector('.admin-factor-details');
  if (checkbox.checked) {
...
/**
 * Initialize safeguards UI
 */
function initializeSafeguards(draftId, draftData) {
  if (!window.DigitalCapExposure) return;

  const container = document.getElementById(`curation-safeguards-container-${draftId}`);
  if (!container) return;

  const safeguards = draftData.recommendedSafeguards || [];

  let html = '';
  safeguards.forEach((sg, idx) => {
    html += renderSafeguardCard(draftId, idx, sg);
  });
...
/**
 * Render a single safeguard card
 */
function renderSafeguardCard(draftId, index, safeguard) {
  const config = window.DigitalCapExposure;
  const sg = safeguard || {};
  ...
  return `
    <div class="admin-safeguard-card" data-draft-id="${draftId}" data-index="${index}" data-safeguard-id="${sg.id || ''}" style="border: 1px solid var(--border-color); border-radius: 6px; padding: 1rem; background: var(--bg-white);">
      ...
      <div style="display: grid; gap: 0.75rem;">
        <div>
          <label style="display: block; font-size: 0.9rem; font-weight: 500; margin-bottom: 0.25rem;">Safeguard Name</label>
          <input type="text" class="admin-safeguard-label" data-draft-id="${draftId}" data-index="${index}" maxlength="100" value="${escapeHtmlAttr(sg.label || '')}" placeholder="e.g., Privacy Settings" style="width: 100%; padding: 0.5rem; border: 1px solid var(--border-color); border-radius: 4px;">
        </div>

        <div>
          <label style="display: block; font-size: 0.9rem; font-weight: 500; margin-bottom: 0.25rem;">Category</label>
          <select class="admin-safeguard-category" data-draft-id="${draftId}" data-index="${index}" style="width: 100%; padding: 0.5rem; border: 1px solid var(--border-color); border-radius: 4px;">
            <option value="">-- Select --</option>
            ${config.safeguardCategories.map(cat => `<option value="${cat}" ${sg.category === cat ? 'selected' : ''}>${cat}</option>`).join('')}
          </select>
        </div>

        <div>
          <label style="display: block; font-size: 0.9rem; font-weight: 500; margin-bottom: 0.25rem;">Type</label>
          <select class="admin-safeguard-type" data-draft-id="${draftId}" data-index="${index}" style="width: 100%; padding: 0.5rem; border: 1px solid var(--border-color); border-radius: 4px;">
            <option value="">-- Select --</option>
            ${config.safeguardTypes.map(t => `<option value="${t}" ${sg.type === t ? 'selected' : ''}>${t}</option>`).join('')}
          </select>
        </div>

        <div>
          <label style="display: block; font-size: 0.9rem; font-weight: 500; margin-bottom: 0.25rem;">Description</label>
          <textarea class="admin-safeguard-description" data-draft-id="${draftId}" data-index="${index}" maxlength="300" placeholder="What does this safeguard do?" style="width: 100%; min-height: 60px; padding: 0.5rem; border: 1px solid var(--border-color); border-radius: 4px;">${escapeHtml(sg.description || '')}</textarea>
        </div>

        <div>
          <label style="display: block; font-size: 0.9rem; font-weight: 500; margin-bottom: 0.25rem;">Instructions</label>
          <textarea class="admin-safeguard-instructions" data-draft-id="${draftId}" data-index="${index}" maxlength="500" placeholder="How to implement this safeguard?" style="width: 100%; min-height: 80px; padding: 0.5rem; border: 1px solid var(--border-color); border-radius: 4px;">${escapeHtml(sg.instructions || '')}</textarea>
        </div>

        <div>
          <label style="display: block; font-size: 0.9rem; font-weight: 500; margin-bottom: 0.25rem;">Impact</label>
          <select class="admin-safeguard-impact" data-draft-id="${draftId}" data-index="${index}" onchange="updateSafeguardFactorsVisibility('${draftId}', ${index})" style="width: 100%; padding: 0.5rem; border: 1px solid var(--border-color); border-radius: 4px;">
            <option value="">-- Select Impact --</option>
            <option value="reduces" ${sg.impactLevel === 'reduces' ? 'selected' : ''}>Reduces (closes pathway)</option>
            <option value="mitigates" ${sg.impactLevel === 'mitigates' ? 'selected' : ''}>Mitigates (reduces harm)</option>
            <option value="manages" ${sg.impactLevel === 'manages' ? 'selected' : ''}>Manages (helps behavior)</option>
          </select>
        </div>

        <div class="admin-safeguard-factors-div" style="display: ${sg.impactLevel === 'reduces' ? 'block' : 'none'};">
          <label style="display: block; font-size: 0.9rem; font-weight: 500; margin-bottom: 0.25rem;">Factors Reduced (only if Impact = "Reduces")</label>
          <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 0.5rem;">
            ${config.factorKeys.map(fk => `
              <label style="display: flex; align-items: center; gap: 0.25rem; font-size: 0.85rem;">
                <input type="checkbox" class="admin-safeguard-factor" data-draft-id="${draftId}" data-index="${index}" data-factor="${fk}" ${factorsSelected.includes(fk) ? 'checked' : ''}>
                ${config.factors[fk].label}
              </label>
            `).join('')}
          </div>
        </div>
        ...
        <div>
          <label style="display: block; font-size: 0.9rem; font-weight: 500; margin-bottom: 0.25rem;">Availability</label>
          <select class="admin-safeguard-availability" data-draft-id="${draftId}" data-index="${index}" style="width: 100%; padding: 0.5rem; border: 1px solid var(--border-color); border-radius: 4px;">
            <option value="">-- Optional / Not Specified --</option>
            <option value="always" ${sg.availability === 'always' ? 'selected' : ''}>Always available</option>
            <option value="some_accounts" ${sg.availability === 'some_accounts' ? 'selected' : ''}>Some accounts</option>
            <option value="requires_premium" ${sg.availability === 'requires_premium' ? 'selected' : ''}>Requires premium</option>
          </select>
        </div>
 ...
 * Add a new safeguard
 */
window.addSafeguard = function(draftId) {
  const container = document.getElementById(`curation-safeguards-container-${draftId}`);
  if (!container) return;

  const newIdx = container.querySelectorAll('.admin-safeguard-card').length;
  const newCard = renderSafeguardCard(draftId, newIdx, {});
 ...
 * Remove a safeguard
 */
window.removeSafeguard = function(draftId, index) {
  const container = document.getElementById(`curation-safeguards-container-${draftId}`);
  if (!container) return;

  const card = container.querySelector(`.admin-safeguard-card[data-index="${index}"]`);
  if (card) card.remove();
};

/**
 * Update safeguard factors visibility based on impact
 */
window.updateSafeguardFactorsVisibility = function(draftId, index) {
  const card = document.querySelector(`.admin-safeguard-card[data-index="${index}"]`);
  if (!card) return;

  const impact = card.querySelector('.admin-safeguard-impact').value;
  const factorsDiv = card.querySelector('.admin-safeguard-factors-div');
  const factorCheckboxes = card.querySelectorAll('.admin-safeguard-factor');
  ...
  if (impact === 'reduces') {
    ...
    // Clear selections
    factorCheckboxes.forEach(cb => cb.checked = false);
  }
};

/**
 * Initialize residual exposure UI
 */
function initializeResidualExposure(draftId, draftData) {
  if (!window.DigitalCapExposure) return;

  const container = document.getElementById(`curation-residual-container-${draftId}`);
  if (!container) return;

  const residual = draftData.residualExposure || [];

  let html = '';
  residual.forEach((re, idx) => {
    html += renderResidualCard(draftId, idx, re);
  });
...
/**
 * Render a single residual exposure card
 */
function renderResidualCard(draftId, index, residual) {
  const config = window.DigitalCapExposure;
  const re = residual || {};

  return `
    <div class="admin-residual-card" data-draft-id="${draftId}" data-index="${index}" style="border: 1px solid var(--border-color); border-radius: 6px; padding: 1rem; background: var(--bg-white);">
      <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 0.75rem;">
        <strong style="font-size: 0.95rem;">Remaining #${index + 1}</strong>
        <button type="button" onclick="removeResidual('${draftId}', ${index})" style="background: var(--danger); color: white; border: none; padding: 0.25rem 0.75rem; border-radius: 4px; cursor: pointer; font-size: 0.8rem;">Remove</button>
      </div>

      <div style="display: grid; gap: 0.75rem;">
        <div>
          <label style="display: block; font-size: 0.9rem; font-weight: 500; margin-bottom: 0.25rem;">Exposure Factor</label>
          <select class="admin-residual-factor" data-draft-id="${draftId}" data-index="${index}" style="width: 100%; padding: 0.5rem; border: 1px solid var(--border-color); border-radius: 4px;">
            <option value="">-- Select Factor --</option>
            ${config.factorKeys.map(fk => `<option value="${fk}" ${re.exposureFactor === fk ? 'selected' : ''}>${config.factors[fk].label}</option>`).join('')}
          </select>
        </div>

        <div>
          <label style="display: block; font-size: 0.9rem; font-weight: 500; margin-bottom: 0.25rem;">What Remains</label>
          <textarea class="admin-residual-statement" data-draft-id="${draftId}" data-index="${index}" maxlength="200" placeholder="What exposure persists?" style="width: 100%; min-height: 60px; padding: 0.5rem; border: 1px solid var(--border-color); border-radius: 4px;">${escapeHtml(re.statement || '')}</textarea>
        </div>

        <div>
          <label style="display: block; font-size: 0.9rem; font-weight: 500; margin-bottom: 0.25rem;">Why It Remains</label>
          <textarea class="admin-residual-reason" data-draft-id="${draftId}" data-index="${index}" maxlength="300" placeholder="Why can't this exposure be reduced?" style="width: 100%; min-height: 80px; padding: 0.5rem; border: 1px solid var(--border-color); border-radius: 4px;">${escapeHtml(re.reason || '')}</textarea>
        </div>

        <div>
          <label style="display: block; font-size: 0.9rem; font-weight: 500; margin-bottom: 0.25rem;">Possible Mitigations</label>
          <textarea class="admin-residual-mitigations" data-draft-id="${draftId}" data-index="${index}" maxlength="300" placeholder="How can users adapt to or manage this?" style="width: 100%; min-height: 60px; padding: 0.5rem; border: 1px solid var(--border-color); border-radius: 4px;">${escapeHtml((re.mitigations || []).join('\n'))}</textarea>
        </div>
      </div>
    </div>
  `;
...
/**
 * Add a new residual exposure
 */
window.addResidualExposure = function(draftId) {
  const container = document.getElementById(`curation-residual-container-${draftId}`);
  if (!container) return;

  const newIdx = container.querySelectorAll('.admin-residual-card').length;
  const newCard = renderResidualCard(draftId, newIdx, {});
  container.insertAdjacentHTML('beforeend', newCard);
};

/**
 * Remove a residual exposure
 */
window.removeResidual = function(draftId, index) {
  const container = document.getElementById(`curation-residual-container-${draftId}`);
  if (!container) return;

  const card = container.querySelector(`.admin-residual-card[data-index="${index}"]`);
  if (card) card.remove();
};
...
window.publishCurationDraft = async function(draftId) {
  ...
  try {
    ...
    const hasV2Work = hasMeaningfulV2DraftData({
      exposureLevel: draft.exposureLevel,
      exposureExplanation: draft.exposureExplanation,
      exposureFactors: draft.exposureFactors,
      exposureFloor: draft.exposureFloor,
      floorRationale: draft.floorRationale,
      recommendedSafeguards: draft.recommendedSafeguards,
      protectedExposureLevel: draft.protectedExposureLevel,
      residualExposure: draft.residualExposure
    });

    if (hasV2Work) {
      // CASE 2: Curator has started v2 work → validate as complete v2
      validationError = getV2ValidationError(draft);
    } else {
      // CASE 1: No v2 work → validate as legacy v1
      validationError = validatePublishable(draft);
    }
...
/**
 * Phase 4E: Validate complete v2 Digital Exposure data with user-friendly diagnostics
 */
function getV2ValidationError(draft) {
  const config = window.DigitalCapExposure;
  if (!config) return 'Unable to load validation configuration';

  // Build temporary validation candidate (schemaVersion only for validation)
  const v2Candidate = {
    ...draft,
    schemaVersion: 2
  };

  // Use shared validator to check completeness
  if (!config.isCompleteV2ExposureData(v2Candidate)) {
    // Provide diagnostic errors for user-friendly messaging

    // Base exposure level
    if (!draft.exposureLevel) return 'Select a Base Digital Exposure Level.';
    if (!config.isValidExposureLevel(draft.exposureLevel)) return 'Base Digital Exposure Level is invalid.';
    if (!draft.exposureExplanation || !draft.exposureExplanation.trim()) {
